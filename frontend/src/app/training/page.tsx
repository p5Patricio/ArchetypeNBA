...
export default function TrainingCampPage() {
  ...
  // Click outside to close player search dropdown
  useEffect(() => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 md:pl-64">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs space-y-3">
          {/* Preset Selector Chips, Intensity & Season */}
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center rounded-xl border border-slate-200 bg-slate-50 p-0.5">
                {(["standard", "high", "elite"] as const).map((lvl) => (
                  <button
                    key={lvl}
                    onClick={() => setTrainingIntensity(lvl)}
                    className={`rounded-lg px-2.5 py-1 text-[11px] font-bold capitalize transition ${
                      trainingIntensity === lvl
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const matchSeason = seasons.find(
                      (s) =>
                        s.season_label.includes(preset.season) ||
                        s.season_label.replace(/\s+/g, "").includes(preset.season.replace(/\s+/g, ""))
                    );
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {analysis.stat_gaps.map((gap) => {
                        ...
                        const isGapSelected = highlightedStat === gap.stat_key;
                        return (
                          <tr
                            key={gap.stat_key}
                            onClick={() => setHighlightedStat(isGapSelected ? null : gap.stat_key)}
                            className={`cursor-pointer transition ${
                              isGapSelected
                                ? "bg-cyan-50/90 font-bold ring-1 ring-inset ring-cyan-400"
                                : "hover:bg-slate-50/70"
                            }`}
                          >
                            <td className="py-2.5 font-bold text-slate-900 flex items-center gap-1.5">
                              {isGapSelected && <span className="h-1.5 w-1.5 rounded-full bg-cyan-600 animate-pulse" />}
                              <span>{language === "es" ? gap.label_es : gap.label_en}</span>
                            </td>
              ...
              {currentRegime && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentRegime.drills.map((drill, idx) => {
                    const isDrillHighlighted =
                      highlightedStat &&
                      (drill.stat_target.toLowerCase().includes(highlightedStat.toLowerCase()) ||
                        highlightedStat.toLowerCase().includes(drill.stat_target.toLowerCase()));
                    return (
                      <div
                        key={drill.id}
                        className={`rounded-2xl border bg-white p-5 shadow-xs flex flex-col justify-between space-y-4 transition ${
                          isDrillHighlighted
                            ? "border-cyan-500 ring-2 ring-cyan-400 bg-cyan-50/25 shadow-md"
